...
from baselines.evaluation_utils import evaluate_binary_classifier, print_score_statistics
...
class ForteTextOODDetector:
    ...
    def _extract_features(self, texts: Sequence[str], name: str = "tmp") -> dict[str, torch.Tensor]:
        ...
        for model_name in models_to_process:
            if batch_accum[model_name]:
                ...
                # Ensure consistent dtype (convert to float32 if needed)
                if all_features[model_name].dtype != torch.float32:
                    all_features[model_name] = all_features[model_name].float()
                ...
                all_features[model_name] = torch.empty(0, device=self.device, dtype=torch.float32)
    ...
    def _compute_nearest_neighbour_distances(
        ...
        effective_k = min(nearest_k + 1, input_features.size(0))
        radii = self._get_kth_value(distances, k=effective_k, axis=-1)
        return radii
    ...
    def fit(
        ...
        if self.method == "gmm":
            ...
            best_n_components = 1
            best_gmm: Any = None
            # Limit components to not exceed number of samples
            n_samples = self.id_train_prdc.shape[0]
            max_components = min(n_samples, 64)
            component_options = [c for c in [1, 2, 4, 8, 16, 32, 64] if c <= max_components]
