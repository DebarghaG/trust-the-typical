...
# (Bill) Adding analysis before detection
from add_analysis import adding_analysis
...
def save_orbench_results_to_csv(
    ...
    for method, results in all_results.items():
        # (Bill) Basic metrics row
        base_row = {
            "Method": method,
            "Embedding_Model": embedding_model,
            "Analysis_Model": analysis_model,  # added by Bill
...
def main(args: argparse.Namespace) -> dict:
    ...
    # (Bill) Here for adding analysis to prompts (safe_train, safe_test, toxic_test)
    max_tokens = 64
    ssafe_train, safe_test, toxic_test = adding_analysis([safe_train, safe_test, toxic_test], args.analysis_model_name, max_tokens)
    logger.info(f"========================== Adding Analysis Done ! ==========================")
    ...
    csv_filepath = save_orbench_results_to_csv(
        all_results, args.embedding_model, args.analysis_model_name, len(safe_train), len(safe_test), len(toxic_test), args
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--methods",
        type=str,
        default="MDJudge_VLLM,RMD,VIM,CIDER,fDBD,NNGuide,ReAct,GMM,AdaScale,OpenMax,Forte_GMM,Forte_OCSVM,LlamaGuard,DuoGuard,PolyGuard_VLLM",
        help="Comma-separated list of methods to run (all models from toxicity detection)",
    )
    ...
    # (Bill) new argument
    parser.add_argument("--analysis_model_name", type=str, default="facebook/opt-125m", help="Adding analysis to prompts.")
