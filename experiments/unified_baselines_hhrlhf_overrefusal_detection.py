#!/usr/bin/env python3
"""
Filename: unified_baselines_hhrlhf_overrefusal_detection.py
Unified Baseline Script for HH-RLHF Overrefusal Detection

Evaluates on the Anthropic HH-RLHF dataset:
- Uses chosen responses as the ID distribution (safe prompts)
- Uses rejected responses as OOD distribution (potentially harmful)
- Includes all models and baselines from toxicity_detection.py
...
logger = logging.getLogger("HHRLHFOverrefusalDetectionBaselines")
...
def extract_prompt_from_conversation(conversation: str, extract_type: str = "human") -> str:
    """Extract human prompt or assistant response from HH-RLHF conversation format."""
    if extract_type == "human":
        # Extract the last human prompt from the conversation
        if "Human:" in conversation:
            # Get all human parts
            human_parts = conversation.split("Human:")
            if len(human_parts) > 1:
                # Get the last human prompt (before the assistant response)
                last_human = human_parts[-1].split("Assistant:")[0].strip()
                return last_human
    elif extract_type == "assistant":
        # Extract the last assistant response
        if "Assistant:" in conversation:
            assistant_parts = conversation.split("Assistant:")
            if len(assistant_parts) > 1:
                # Get the last assistant response
                last_assistant = assistant_parts[-1].strip()
                return last_assistant

    # Fallback: return first 500 characters if extraction fails
    return conversation[:500].strip()


def load_hhrlhf_data(
    max_samples: int = 10000, split: str = "train", use_rejected_as_toxic: bool = True
) -> tuple[list[str], list[str], dict]:
    """
    Load HH-RLHF data and separate chosen vs rejected prompts.

    Args:
        max_samples: Maximum number of samples to load
        split: Dataset split to use ('train' or 'test')
        use_rejected_as_toxic: If True, use rejected responses as toxic/OOD samples

    Returns:
        Tuple of (chosen_prompts, rejected_prompts, metadata)
    """
    logger.info(f"Loading HH-RLHF data from split: {split}...")

    chosen_prompts: list[str] = []  # ID distribution (chosen/preferred responses)
    rejected_prompts: list[str] = []  # OOD distribution (rejected responses)
    ...
    try:
        ...
        logger.info(f"Loaded {len(dataset)} examples from HH-RLHF {split} split")

        chosen_lengths = []
        rejected_lengths = []

        for idx, example in enumerate(dataset):
            if len(chosen_prompts) >= max_samples and len(rejected_prompts) >= max_samples:
                break

            # Extract chosen and rejected conversations
            chosen_conv = example.get("chosen", "")
            rejected_conv = example.get("rejected", "")

            # Extract human prompts from conversations
            # We focus on the human prompts as they represent the requests
            chosen_human = extract_prompt_from_conversation(chosen_conv, "human")
            rejected_human = extract_prompt_from_conversation(rejected_conv, "human")

            # Add to appropriate lists
            if chosen_human and len(chosen_human.strip()) > 20:
                if len(chosen_prompts) < max_samples:
                    chosen_prompts.append(chosen_human.strip())
                    chosen_lengths.append(len(chosen_human))

            if use_rejected_as_toxic and rejected_human and len(rejected_human.strip()) > 20:
                if len(rejected_prompts) < max_samples:
                    rejected_prompts.append(rejected_human.strip())
                    rejected_lengths.append(len(rejected_human))
        ...
        # Calculate metadata
        metadata["total_samples"] = len(dataset)
        metadata["chosen_count"] = len(chosen_prompts)
        metadata["rejected_count"] = len(rejected_prompts)
        metadata["avg_chosen_length"] = np.mean(chosen_lengths) if chosen_lengths else 0
        metadata["avg_rejected_length"] = np.mean(rejected_lengths) if rejected_lengths else 0

    except Exception as e:
        logger.error(f"Could not load HH-RLHF dataset: {e}")
        return [], [], metadata

    logger.info(f"Loaded {len(chosen_prompts)} chosen prompts (ID)")
    logger.info(f"Loaded {len(rejected_prompts)} rejected prompts (OOD)")
    logger.info(f"Average chosen prompt length: {metadata['avg_chosen_length']:.1f} chars")
    logger.info(f"Average rejected prompt length: {metadata['avg_rejected_length']:.1f} chars")

    return chosen_prompts, rejected_prompts, metadata


def load_combined_hhrlhf_data(
    max_safe_samples: int = 5000, max_toxic_samples: int = 2000, train_test_split: float = 0.7
) -> tuple[list[str], list[str], list[str]]:
    """
    Load combined HH-RLHF data: chosen prompts for training, chosen + rejected for testing.

    Args:
        max_safe_samples: Maximum number of safe/chosen samples to load
        max_toxic_samples: Maximum number of toxic/rejected samples to load
        train_test_split: Proportion of safe samples to use for training

    Returns:
        Tuple of (safe_train, safe_test, toxic_test)
    """
    logger.info("Loading combined HH-RLHF data...")

    # Load chosen and rejected prompts from train split
    chosen_prompts, rejected_prompts, metadata = load_hhrlhf_data(
        max_samples=max_safe_samples, split="train", use_rejected_as_toxic=True
    )

    # If we need more samples, also load from test split
    if len(chosen_prompts) < max_safe_samples or len(rejected_prompts) < max_toxic_samples:
        logger.info("Loading additional samples from test split...")
        test_chosen, test_rejected, test_metadata = load_hhrlhf_data(
            max_samples=max_safe_samples - len(chosen_prompts),
            split="test",
            use_rejected_as_toxic=True,
        )
        chosen_prompts.extend(test_chosen)
        rejected_prompts.extend(test_rejected)

    # Limit to requested maximums
    chosen_prompts = chosen_prompts[:max_safe_samples]
    rejected_prompts = rejected_prompts[:max_toxic_samples]

    # Split chosen prompts into train/test
    train_size = int(len(chosen_prompts) * train_test_split)
    safe_train = chosen_prompts[:train_size]
    safe_test = chosen_prompts[train_size:]

    # Use rejected prompts as toxic test set
    toxic_test = rejected_prompts

    logger.info(f"Training set: {len(safe_train)} safe/chosen prompts")
    logger.info(f"Test set: {len(safe_test)} safe + {len(toxic_test)} toxic/rejected prompts")

    return safe_train, safe_test, toxic_test


def create_postprocessor(method: str, args: argparse.Namespace) -> Any:
    """Create postprocessor based on method."""
    embedding_dir = f"embeddings_{method.lower()}_hhrlhf_overrefusal"
    ...
    if method.lower().startswith("forte_"):
        ...
        return LlamaGuardLogitTextPostprocessor(
            ...
            batch_size=args.batch_size,
            device=args.device,
            embedding_dir=embedding_dir.replace("hhrlhf_", "hhrlhf_logit_"),
            max_new_tokens=args.llama_guard_max_tokens,
            temperature=args.llama_guard_temperature,
        )
        ...
        return WildGuardTextPostprocessor(
            ...
            batch_size=args.batch_size,
            device=args.device,
            embedding_dir=embedding_dir.replace("hhrlhf_", "wildguard_hhrlhf_"),
...
def calculate_rejection_rates(
    ...
    """Calculate rejection rates at different thresholds for overrefusal analysis.
...
def run_hhrlhf_evaluation(
    ...
    """Run evaluation on HH-RLHF overrefusal detection."""
    logger.info(f"Evaluating {method} on HH-RLHF...")
    ...
    if method.lower().startswith("forte_"):
        ...
        safe_scores = detector.postprocess(
            safe_test_prompts, cache_name=f"{method.lower()}_hhrlhf_safe_test"
        )

        toxic_scores = detector.postprocess(
            toxic_test_prompts, cache_name=f"{method.lower()}_hhrlhf_toxic_test"
        )
        ...
        # Calculate rejection rates for overrefusal analysis
        rejection_rates = calculate_rejection_rates(scores_all, labels)
        ...
        # Clean up memory after evaluation for VLLM-based models
        if method.lower() in ["mdjudge_vllm", "polyguard_vllm"]:
            logger.info(f"Cleaning up memory after {method} evaluation...")
            cleanup_vllm_memory(method_name=f"{method}_post")

        return results


def save_hhrlhf_results_to_csv(
    all_results: dict,
    embedding_model: str,
    num_safe_train: int,
    num_safe_test: int,
    ...
    """Save HH-RLHF results to CSV file in results directory."""
    ...
    filename = f"hhrlhf_overrefusal_detection_results_{timestamp}.csv"
...
def main(args: argparse.Namespace) -> dict:
    ...
    logger.info(f"Running HH-RLHF overrefusal detection baselines with configuration: {args}")

    # Load HH-RLHF data
    logger.info("\n=== Phase 1: Loading HH-RLHF Data ===")
    safe_train, safe_test, toxic_test = load_combined_hhrlhf_data(
        max_safe_samples=args.max_safe_samples,
    ...
    for method in methods_to_run:
        logger.info(f"\n{'='*60}")
        logger.info(f"Running {method} Method on HH-RLHF")
        logger.info(f"{'='*60}")
        ...
        logger.info(f"\n=== {method}: HH-RLHF Evaluation ===")
        results = run_hhrlhf_evaluation(detector, safe_test, toxic_test, method, args)
        all_results[method] = results

        logger.info(
            f"{method} on HH-RLHF: AUROC={results['AUROC']:.4f}, FPR@95={results['FPR@95TPR']:.4f}"
        )
    ...
    # Print summary table
    logger.info(f"\n{'='*80}")
    logger.info("HH-RLHF OVERREFUSAL DETECTION RESULTS SUMMARY")
    logger.info(f"{'='*80}")
    logger.info("Dataset: Anthropic/hh-rlhf")
    ...
    # Header
    header = f"{'Method':<20}{'AUROC':<8}{'FPR@95':<8}{'AUPRC':<8}{'F1':<8}"
    logger.info(header)
    ...
    for method in methods_to_run:
        if method in all_results:
            ...
            row = f"{method:<20}{results['AUROC']:<8.3f}{results['FPR@95TPR']:<8.3f}{results['AUPRC']:<8.3f}{results['F1']:<8.3f}"
            logger.info(row)

    # Print HH-RLHF style rejection rate table (similar to OR-Bench)
    logger.info(f"\n{'='*80}")
    logger.info("HH-RLHF REJECTION RATE TABLE")
    logger.info(f"{'='*80}")
    logger.info(
        "Shows rejection rates for Safe/Chosen prompts (should be low) vs Toxic/Rejected prompts (should be high)"
    )
    logger.info("")
    ...
    if has_rejection_data:
        ...
        header_rr = f"{'Method':<20}{'Threshold':<12}{'Safe Rejection %':<16}{'Toxic Rejection %':<17}{'Accuracy':<10}"
        ...
        for method in methods_to_run:
            if method in all_results and "rejection_rates" in all_results[method]:
                ...
                row_rr = f"{method:<20}{threshold:<12.3f}{safe_reject_pct:<16.1f}{toxic_reject_pct:<17.1f}{accuracy:<10.3f}"
                logger.info(row_rr)
    ...
    # Save results to CSV
    csv_filepath = save_hhrlhf_results_to_csv(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Unified Baselines - HH-RLHF Overrefusal Detection"
    )
    ...
    # Data parameters
    parser.add_argument(
        "--max_safe_samples", type=int, default=5000, help="Max safe/chosen prompts to load"
    )
    parser.add_argument(
        "--max_toxic_samples", type=int, default=2000, help="Max toxic/rejected prompts to load"
    )
    parser.add_argument(
        "--train_test_split",
        type=float,
        default=0.7,
        help="Proportion of safe samples to use for training (0-1)",
    )
    # General parameters
